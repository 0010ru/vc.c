#include <stdio.h> // TODO: Hide for users
#include <stdlib.h>
#include <stdint.h>

#include <X11/Xlib.h> // TODO: Hide for users
...
typedef struct {
    Display *display; // TODO: Display type to VC_Display type
    Window window; // TODO: Need to switch to smthing "VC_" else
...
VC_EventType vc_get_next_event(Display *display);
...
VC_EventType vc_get_next_event_x11(Display *display);
...
VC_Window vc_create_window_x11(VC_WindowParams *params) {
    ...
    return (VC_Window) {x_display, x_window, params};
}
...
VC_EventType vc_get_next_event(Display *display) {
    return vc_get_next_event_x11(display);
}
...
void vc_run_event_loop(VC_Window *window) {
    int running = 1;
    while (running) {
        VC_EventType event_type = vc_get_next_event(window->display); 
